...
#Saving file         
    ...
    def write_new_user_data(self, username, password) -> None:
        ...
        return True
# For Checj=king User Exist or Not in File Database   
...
#For giving password to existing email   or user name
...
def main():
            ...
            if user_choice == 1:
                ...
                if username == "EXIT":
                    ...
                    if user_creation_status:
                        print_message("USER CREATED SUCCESSFULL")   #print command if user registered
                    else:
                        print_message("USERNAME ALREADY EXISTS")    #printing command if user already exist in File Database
                        
                else:
                    print_message("ENTER VALID DETAILS")      #command for, if the user creds are wrong 
                ...
                if username == "EXIT":
                    ...
                    if login_status:
                        print_message("LOGIN SUCCESSFUL")    #if Login Successfull
                    else:
                        print_message("INVALID CREDS")       #for invalid creds
                else:
                    print_message("ENTER VALID DETAILS")     #to enter valid details if logib not successfull
  ...
  #for entering password to email or giving password to existing email or username                      
                        if creds_check:
                            while True:
